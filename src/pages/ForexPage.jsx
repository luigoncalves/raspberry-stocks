...
function ForexPage() {
  ...
  const getHistoricalData = async stockTicker => {
    try {
      const response = await axios.get(
        `https://financialmodelingprep.com/api/v3/historical-price-full/${forexTicker}?apikey=${apiKey}`
      );
  ...
  const forexPriceChanges = async () => {
    try {
      const response = await axios.get(
        `https://financialmodelingprep.com/api/v3/stock-price-change/${forexTicker}?apikey=${apiKey}`
      );
